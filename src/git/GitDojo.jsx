  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FAFBFC]">
      <div className="flex-none flex items-center justify-between px-4 sm:px-6 py-3 bg-white/80 backdrop-blur-sm border-b border-[#EDE3D2] shadow-sm sticky top-0 z-10">
        ...
        <div className="text-xs text-slate-500">Kata V · Gestion de versions</div>
      </div>
                  ...
                  {exs.length > 0 && (
                    <div className="flex-1 min-w-[120px] flex items-center gap-2 ml-2">
                      <div className="flex-1 h-1.5 bg-[#F4EADB] rounded-full overflow-hidden">
                        <div className="h-full bg-[#059669] rounded-full transition-all duration-500" style={{ width: `${(completedCount / exs.length) * 100}%` }} />
                      </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          {ex.isTutorial && stars === 0 && (
                            <span className="absolute -top-2 -right-2 bg-[#059669] text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-[0_2px_0_rgba(0,0,0,0.08)]">TUTO</span>
                          )}
                        </button>
                      );
                    })}
...
export default function GitDojo({ onBackToHub }) {
  ...
  return (
    <div className="relative">
      ...
      {currentExercise && (
        <div className="fixed top-[52px] left-44 right-0 bg-emerald-50 border-b border-emerald-200 px-4 py-1.5 flex items-center justify-between z-10">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-xs font-bold text-emerald-800 shrink-0">{currentExercise.title}</span>
            <span className="text-xs text-emerald-600 truncate">{currentExercise.description}</span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button onClick={() => setShowPrompt(true)}
              className="text-xs text-emerald-700 font-medium px-2 py-1 rounded hover:bg-emerald-100 flex items-center gap-1">
        ...
        <div className="fixed inset-0 modal-overlay flex items-center justify-center z-50" onClick={() => setShowPrompt(false)}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-[28rem] max-w-[90vw] modal-content" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-display text-2xl text-emerald-800">{currentExercise.title}</h3>
            </div>
            <p className="text-sm text-slate-700 leading-relaxed mb-4 whitespace-pre-line">{currentExercise.prompt}</p>
            <button onClick={() => setShowPrompt(false)} className="w-full py-2.5 rounded-xl bg-emerald-500 text-white font-bold hover:bg-emerald-600 shadow">C'est parti</button>
          </div>
        </div>
      )}
          ...
          <div className="fixed inset-0 modal-overlay flex items-center justify-center z-50" onClick={() => setValidationResult(null)}>
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-96 text-center modal-content" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setValidationResult(null); handleBack(); }} className="flex-1 py-2 rounded-lg bg-emerald-500 text-white font-bold hover:bg-emerald-600 shadow">Suivant</button>
              </div>
            </div>
          </div>
        ) : (
